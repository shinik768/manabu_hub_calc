...
def compute_intercepts(left_expr, right_expr, x, y):
    ...
    # 出力を生成
    x_intercepts_str = (f'{x} = ' + ', '.join(map(str, x_real_intercepts))) if x_real_intercepts else '存在しないか、求めるのに時間がかかりすぎます'
    y_intercepts_str = (f'{y} = ' + ', '.join(map(str, y_real_intercepts))) if y_real_intercepts else '存在しないか、求めるのに時間がかかりすぎます'
